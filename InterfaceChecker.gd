...
func check_nodes(node:Node): 
	...
	if "interfaces" in node: 
		for interface:Script in node.interfaces: 
			var required_methods = interface.get_script_method_list().map(get_names)
			var implemented_methods = node.get_script().get_script_method_list().map(get_names)
